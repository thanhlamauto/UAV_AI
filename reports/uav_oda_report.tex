...
External probe & Multi-LiDAR có 26 sequence, trong đó 8 hard; ARCO có 3 ZIP direct, nhỏ nhất khoảng \(0.714\,GiB\) \\
...
\centering
\small
\caption{Imbalance-aware future-risk classification trên 1096 frame, split theo trial ID.}
...
Sau khi chuyển sang batch inference và load model một lần, DPT xử lý 1096 frame với trung bình \(0.0886\,s/frame\) wall-time và \(0.0423\,s/frame\) inference-time. ONNX Runtime CUDA đã chạy được trên 3 trial mẫu, đạt \(0.0827\,s/frame\) wall-time và \(0.0405\,s/frame\) inference-time. TensorRT chưa được claim vì server thiếu \texttt{trtexec} và thiếu thư viện \texttt{libnvinfer.so.10}.
...
python3 scripts/create_oda_trial_manifest.py --total 300 --zip-path /workspace/data/Dupeyroux_et_al_2021_ODA_DATASET.zip
python3 scripts/extract_oda_trials_from_full_zip.py /workspace/data/Dupeyroux_et_al_2021_ODA_DATASET.zip
scripts/run_oda_manifest_benchmark.sh outputs_300/tables/target_300_trials_manifest.csv outputs_300
...
scripts/export_depth_to_onnx.sh Intel/dpt-hybrid-midas models/depth_onnx/dpt_hybrid_midas
python3 experiments/cache_monocular_depth_onnx.py --provider cuda --trial-ids 3 10 345
python3 experiments/build_perception_risk_features.py
python3 experiments/train_perception_risk_ablation.py --resample-train oversample --optimize-threshold
...
Mức độ hoàn thiện & Có output cụ thể: bảng CSV, hình PNG, video MP4, mã nguồn module hóa, batch benchmark 300 trial, risk feature 1096 dòng, depth/ONNX comparison và lệnh reproduce. \\
Hiệu quả tính toán & Metric trajectory chạy rất nhanh; A*/RRT ở mức vài ms/trial, MPPI khoảng \(39.93\,ms\), RRT* Python khoảng \(1196.40\,ms\). Batch depth DPT đạt \(0.0886\,s/frame\), ONNX CUDA probe đạt \(0.0827\,s/frame\); chưa claim TensorRT/onboard real-time. \\
\bottomrule
\end{tabularx}
\end{table}
...
Hướng phát triển tiếp theo gồm: chạy depth/imbalance trên nhiều hơn 20 trial, dùng TensorRT container để đo engine thật, và tải một sample ARCO/Multi-LiDAR để stress-test sensing. Multi-LiDAR/ARCO chỉ nên dùng để chứng minh tổng quát hóa và độ khó perception; ODA vẫn là benchmark UAV obstacle-avoidance chính. Riêng depth panel cần được tiếp tục hiệu chuẩn với ground truth/radar trước khi dùng như metric depth.
