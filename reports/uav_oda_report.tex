...
External probe & ARCO đã tải/probe 3 ROS2 bag ZIP, 33 topic rows, 175997 messages; Multi-LiDAR có 27/27 link SharePoint cần đăng nhập \\
...
Mã nguồn và script tái lập kết quả nằm trong cùng thư mục dự án. Các lệnh chính được gom theo pipeline:
\begin{lstlisting}[basicstyle=\ttfamily\footnotesize,breaklines=true,frame=single]
scripts/download_oda_full_zip.sh ...
python3 scripts/create_oda_trial_manifest.py --total 300 ...
scripts/run_oda_manifest_benchmark.sh ... outputs_300
python3 experiments/cache_monocular_depth_batch.py --model-id depth-anything/Depth-Anything-V2-Small-hf ...
...
\subsection{Đối chiếu yêu cầu bài toán}
\begin{table}[H]
\centering
\scriptsize
\caption{Đối chiếu checklist đề bài với phạm vi đã triển khai.}
\begin{tabularx}{0.98\linewidth}{>{\bfseries}p{2.9cm} X X}
\toprule
Yêu cầu & Đã thực hiện & Ghi chú/phạm vi \\
\midrule
Dữ liệu camera/radar/LiDAR & RGB, radar, IMU và OptiTrack của ODA; ARCO/Multi-LiDAR được probe ngoài ODA. & ODA không phải benchmark LiDAR chính; LiDAR/point cloud dùng để chứng minh hướng stress-test. \\
Phát hiện vật cản & Monocular relative depth, radar FFT/peak/energy, clearance label từ ground truth. & Chưa làm object detection, point-cloud segmentation hoặc full Range-Doppler tracking. \\
Biểu diễn vật cản & Safety circle, safety boundary và occupancy grid cho A*. & Chưa dùng bounding box/voxel; phù hợp với metadata hình học của ODA. \\
Lập kế hoạch tránh vật cản & A* search, RRT/RRT* sampling-based, MPPI optimization-based/sampling optimal control. & MPC/ROS controller chưa đưa vào để giữ pipeline nhẹ và tái lập nhanh. \\
Đánh giá & Collision, safety violation, min clearance, path length, smoothness heading-change, computation time. & Tính khả thi động học mới ở mức proxy; jerk/acceleration là hướng mở rộng. \\
\bottomrule
\end{tabularx}
\end{table}
...
Mức độ hoàn thiện & Có output cụ thể: bảng CSV, hình PNG, video MP4, mã nguồn module hóa, batch benchmark 300 trial, risk feature 2584 dòng trên 50 trial, depth/ONNX/TensorRT-provider probe và lệnh reproduce. \\
Hiệu quả tính toán & Metric trajectory chạy rất nhanh; A*/RRT ở mức vài ms/trial, MPPI khoảng \(39.93\,ms\), RRT* Python khoảng \(1196.40\,ms\). Depth Anything V2 Small batch đạt \(0.0639\,s/frame\) wall-time và \(0.0174\,s/frame\) inference-time; ONNX CUDA probe đạt \(0.0827\,s/frame\). TensorRT engine thật chưa claim do thiếu TensorRT runtime trên container hiện tại. \\
\bottomrule
\end{tabularx}
\end{table}
