...
DURATION_S="${DURATION_S:-28}"
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
OUT_DIR="${REPO_ROOT}/outputs/nvblox_pointcloud_slice_planner_runtime/$(date +%Y%m%d_%H%M%S)"

mkdir -p "${OUT_DIR}"

log() {
  printf '[nvblox-pointcloud-verify] %s\n' "$*" | tee -a "${OUT_DIR}/verify.log"
}
...
cd "${REPO_ROOT}"
log "Launching synthetic organized PointCloud2 -> NVBlox DistanceMapSlice -> MPPI planner"
setsid ros2 launch uav_oda_ros2_demo nvblox_pointcloud_slice_planner.launch.py \
...
timeout 8 ros2 topic echo --once --full-length /nvblox_distance_slice_planner/status >"${OUT_DIR}/status_echo.txt" 2>&1 || true

cleanup
unset LAUNCHER_PID

cp "${OUT_DIR}/topics.txt" "${REPO_ROOT}/outputs/nvblox_pointcloud_slice_topics.txt"
cp "${OUT_DIR}/lidar_points_echo.txt" "${REPO_ROOT}/outputs/nvblox_pointcloud_lidar_points_echo.txt"
cp "${OUT_DIR}/static_map_slice_echo.txt" "${REPO_ROOT}/outputs/nvblox_pointcloud_static_map_slice_echo.txt"
cp "${OUT_DIR}/planned_path_echo.txt" "${REPO_ROOT}/outputs/nvblox_pointcloud_planned_path_echo.txt"
cp "${OUT_DIR}/status_echo.txt" "${REPO_ROOT}/outputs/nvblox_pointcloud_status_echo.txt"
cp "${OUT_DIR}/launch.log" "${REPO_ROOT}/outputs/nvblox_pointcloud_planner_ros2.log"
...
grep -q "height: 16" "${OUT_DIR}/lidar_points_echo.txt"
grep -q "width: 360" "${OUT_DIR}/lidar_points_echo.txt"
grep -q "poses:" "${OUT_DIR}/planned_path_echo.txt"

for required in lidar_points_echo.txt static_map_slice_echo.txt planned_path_echo.txt status_echo.txt; do
...
log "PointCloud2 LiDAR -> NVBlox DistanceMapSlice planner verification PASSED"
