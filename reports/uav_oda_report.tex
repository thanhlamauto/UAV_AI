...
\begin{itemize}
  ...
  \item \texttt{experiments/analyze\_radar\_range\_doppler.py}, \texttt{analyze\_obstacle\_representations.py}, \texttt{analyze\_trajectory\_feasibility.py}: bổ sung range-Doppler, representation và feasibility proxy.
\end{itemize}
...
\midrule
Dữ liệu camera/radar/LiDAR & RGB, radar, IMU và OptiTrack của ODA; ARCO probe có 26324 point-cloud message, Multi-LiDAR 27 link cần đăng nhập. & ODA không phải benchmark LiDAR chính; point cloud dùng làm stress evidence, chưa là pipeline chính. \\
Phát hiện vật cản & Monocular relative depth, radar FFT/peak/energy, radar range-Doppler \(16\times128\) trên 3 sample, clearance label từ ground truth. & Chưa làm object detection hoặc point-cloud segmentation/clustering. \\
Biểu diễn vật cản & Safety circle, safety boundary, 2D bounding box và occupancy footprint trên 300 trial; hai vật cản có bbox trung bình \(5.838\,m^2\), occupancy \(3.080\,m^2\). & Chưa dùng voxel/3D bounding box vì ODA metadata là hình học 2D trên mặt phẳng bay. \\
...
Đánh giá & Collision, safety violation, min clearance, path length, smoothness heading-change, speed proxy và computation time. & Tính khả thi động học mới ở mức proxy; jerk/acceleration/actuator limit là hướng mở rộng. \\
\bottomrule
\end{tabularx}
\end{table}
